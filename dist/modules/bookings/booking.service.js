...
const createBooking = async (loggedUser, bookingPayload) => {
  if (loggedUser.role === user_enum_1.ENUM_USER_ROLE.CUSTOMER) {
    ...
    if (
      !(0, booking_utils_1.areBothTimesInBetween)(
    ...
    for (const slot of existingBookingSlots) {
      const existingBookingSlotStartTime = (0, moment_1.default)(
        slot.serviceStartTime,
        'HH:mm',
      )
      const existingBookingSlotEndTime = (0, moment_1.default)(
        slot.serviceEndTime,
        'HH:mm',
      )
      const newBookingSlotStartTime = (0, moment_1.default)(
        bookingPayload.serviceStartTime,
        'HH:mm',
      )
      const newBookingSlotEndTime = (0, moment_1.default)(
        bookingPayload.serviceEndTime,
        'HH:mm',
      )
      if (
...
const updateBooking = async (loggedUser, bookingId, updatePayload) => {
  ...
  const updateBooking = await booking_model_1.default.findByIdAndUpdate(
    { _id: bookingId },
    { ...updatePayload },
    { new: true },
  )
...
const getAllBookings = async (loggedUser, queryOptions, filterOptions) => {
  ...
  if (
    loggedUser.role === user_enum_1.ENUM_USER_ROLE.ADMIN ||
  ...
  const { page, limit, skip, sortBy, sortOrder } =
    pagination_1.paginationHelpers.calculatePagination(queryOptions)
  const sortCondition = {}
  if (sortBy && sortOrder) {
  ...
  if (searchTerm) {
    ...
    bookings = await booking_model_1.default.aggregate(
      (0, booking_utils_1.bookingsAggregationPipeline)(nameRegex),
    )
  } else {
    bookings = await booking_model_1.default
      .find(queryPayload)
      .sort(sortCondition)
      .limit(limit)
      .skip(skip)
