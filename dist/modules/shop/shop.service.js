...
const createShop = async (loggedUser, shopPayload) => {
  if (loggedUser.role === user_enum_1.ENUM_USER_ROLE.SELLER) {
    const shop = await shop_model_1.default.findOneAndUpdate(
...
const updateShop = async (loggedUser, shopId, updatePayload) => {
  ...
  const updateShop = await shop_model_1.default.findByIdAndUpdate(
    { _id: shopId },
    { ...updatePayload },
    { new: true },
  )
...
const getAllShop = async (loggedUser, queryOptions, filterOptions) => {
  if (
    ...
    const shopList = await shop_model_1.default
      .find(queryPayload)
      .sort(sortCondition)
      .skip(skip)
      .limit(limit)
