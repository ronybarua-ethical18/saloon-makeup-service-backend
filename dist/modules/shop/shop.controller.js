...
const createShop = (0, tryCatchAsync_1.default)(async (req, res) => {
  ...
  const result = await shop_service_1.ShopService.createShop(
    loggedUser,
    req.body,
  )
...
const getAllShop = (0, tryCatchAsync_1.default)(async (req, res) => {
  ...
  const filterOptions = (0, pick_1.default)(
    req.query,
    shop_constants_1.shopFilterableFields,
  )
  const queryOptions = (0, pick_1.default)(
    req.query,
    pagination_1.paginationFields,
  )
  const result = await shop_service_1.ShopService.getAllShop(
    loggedUser,
    queryOptions,
    filterOptions,
  )
...
const getShop = (0, tryCatchAsync_1.default)(async (req, res) => {
  if (typeof req.params.shopId === 'string') {
    const result = await shop_service_1.ShopService.getShop(
      new mongoose_1.default.Types.ObjectId(req.params['shopId']),
    )
    ;(0, sendResponse_1.default)(res, {
      statusCode: 200,
      success: true,
...
const updateShop = (0, tryCatchAsync_1.default)(async (req, res) => {
  const loggedUser = req.user
  if (typeof req.params.shopId === 'string') {
    const result = await shop_service_1.ShopService.updateShop(
      loggedUser,
      new mongoose_1.default.Types.ObjectId(req.params['shopId']),
      req.body,
    )
...
const deleteShop = (0, tryCatchAsync_1.default)(async (req, res) => {
  const loggedUser = req.user
  if (typeof req.params.shopId === 'string') {
    await shop_service_1.ShopService.deleteShop(
      loggedUser,
      new mongoose_1.default.Types.ObjectId(req.params['shopId']),
    )
    ;(0, sendResponse_1.default)(res, {
