...
const getService = async serviceId => {
  const service = await service_model_1.ServiceModel.findById({
    _id: serviceId,
  })
  if (!service) {
    throw new ApiError_1.default(
...
const updateService = async (loggedUser, serviceId, updatePayload) => {
  ...
  const updateService = await service_model_1.ServiceModel.findByIdAndUpdate(
    { _id: serviceId },
    { ...updatePayload },
    { new: true },
  )
...
const deleteService = async (loggedUser, serviceId) => {
  ...
  const service =
    await service_model_1.ServiceModel.findOneAndDelete(queryPayload)
  if (!service) {
    throw new ApiError_1.default(
      http_status_1.default.NOT_FOUND,
      'Service not found',
...
const getAllServices = async (loggedUser, queryOptions, filterOptions) => {
  ...
  const queriesWithFilterableFields = (0,
  queryFieldsManipulation_1.queryFieldsManipulation)(
    searchTerm,
  ...
  const services = await (0, workerHandler_1.createWorker)(
    queryPayload,
    sortCondition,
    skip,
    limit,
