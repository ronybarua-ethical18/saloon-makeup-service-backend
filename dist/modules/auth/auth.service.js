...
const signUpUser = async payload => {
  ...
  ;(0, auth_utils_1.sendMailWithToken)(
    newUser,
...
const loginUser = async payload => {
  ...
  const isPasswordMatched = await user_model_1.UserModel.isPasswordMatched(
    password,
    user.password,
  )
...
const forgotPassword = async email => {
  ...
  ;(0, auth_utils_1.sendMailWithToken)(
    ...
    'reset-password',
    constants_1.FORGOT_PASSWORD_TEMPLATE,
  )
  return user
}
const resetPassword = async payload => {
  const { userId } = await jwtHelpers_1.jwtHelpers.verifyToken(
    payload.token,
    config_1.default.jwt.secret,
  )
  const user = await user_model_1.UserModel.findById({ _id: userId })
